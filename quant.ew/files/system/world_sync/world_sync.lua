...
local function get_all_chunks(ocx, ocy, pos_data, priority, give_0)
    ...
    local header_end  = string.char(math.min(priority + 1, 16))..pos_data
    local header_end_2  = string.char(math.min(priority + 2, 16))..pos_data
    ...
    elseif frame % (int * 16) == 7 then
        ...
        net.proxy_bin_send(KEY_WORLD_END, header_end_2)
        iter_slow = (iter_slow + 1) % #slow_offsets
    elseif (priority == 0 and not GameHasFlagRun("ending_game_completed"))
           and frame % (int * 3) == 1 then
